...
N_Matrix_N_Mods_run_East<-lapply(N_Matrix_N_Formula,function (x) rq(x, tau=0.9, data=SOURCE))

	## Make null models
N_Matrix_N_intercept_East<-rq(formula = N_Matrix ~ 1, tau = 0.9, data = SOURCE, model = T)

	## Grab each model fit but the intercept is hard coded 

N_Matrix_N_Mods_fit_East<-mapply(function (y) 1-N_Matrix_N_Mods_run_East[[y]]$rho/N_Matrix_N_intercept_East$rho, 1:length(XVAR))

	## makes a list of mod names

N_Matrix_N_Mod_names<-lapply(XVAR_names, function (x) paste("N_Matrix_",paste(x),sep=""))

names(N_Matrix_N_Mods_run_East)<-N_Matrix_N_Mod_names

	## Save model stats as a spreadsheet of manuscript

N_Matrix_N_Mods_AIC<-lapply(N_Matrix_N_Mods_run_East,AIC)

N_Matrix_N_Lichen_Count_vs_N_Stats<-cbind(XVAR_names,N_Matrix_N_Mods_AIC,N_Matrix_N_Mods_fit_East,paste(N_Matrix_N_Formula))
...
N_Matrix_N_Mod_pred_East<-mapply(MyFunc2,N_Matrix_N_Mods_run_East)%>%as.data.frame()

	## This works but is hard coded for each predictor file. Have to change working directory each time

pdf(paste("N_Matrix_N_Lichen_Count_vs_N_percentiles_East","_",format(Sys.time(),"%y%d%m"),".pdf",sep=""))

MyPlot<-function (x)
    
{
    x<-N_Matrix_N_Mod_pred_East[i]
    ...
    text(max(SOURCE$N)*0.7,max(SOURCE$N_Matrix)*0.85,labels=paste("R1=",print(round(N_Matrix_N_Mods_fit_East[i],2))))
    text(max(SOURCE$N)*0.7,max(SOURCE$N_Matrix)*0.8,labels=paste(as.character(N_Matrix_N_Formula[i])), cex=0.7)
    legend('topright', legend=c("Raw Data East","Fitted Values", "Prediction Interval"), col=c("darkorange","blue", "black"), pch=16)
...
dev.off()
    ...
    x<-N_Matrix_N_Mod_pred_East[2]
    x2<-unlist(x, recursive = F, use.names =T)
	  y<-N_Matrix_N_Mod_pred_East[4]
    ...
    #text(max(SOURCE$N)*0.8,max(SOURCE$N_Matrix)*0.7,labels=paste("R1=",print(round(N_Matrix_N_Mods_fit_East[2],2))))
...
###### and the fitted polynomial line with the predicted fitted 
	...
	five_pct_N_Matrix  <- max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)-(max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)*0.05) %>% as.data.frame()
	ten_pct_N_Matrix   <- max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)-(max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)*0.1) %>% as.data.frame()
	twenty_pct_N_Matrix<- max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)-(max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)*0.2) %>% as.data.frame()
	fifty_pct_N_Matrix <- max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)-(max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)*0.5) %>% as.data.frame()
	eighty_pct_N_Matrix<- max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)-(max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)*0.8) %>% as.data.frame()
	...
	,eighty_pct_N_Matrix))
		...
		## x - 5
		## y - N_Matrix_N_Mods_run_East
		## ModRun - N_Matrix_N_poly
		## ModPred N_Matrix_N_Mod_pred_East
	...
	#}
	#GetYforX(5,N_Matrix_N_Mods_run_East,N_Matrix_N_poly,N_Matrix_N_Mod_pred_East)
			
			test_n<-as.data.frame(5)
			colnames(test_n)<-"N"
			1-(predict(N_Matrix_N_Mods_run_East$N_Matrix_N_poly, test_n)/max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit))
		...
		## Heuristically solved for X given a Y using the polynomial 
			## Heuristically solved for X given a Y ...	34.58681
				...
				c(
				 1-(predict(N_Matrix_N_Mods_run_East$N_Matrix_N_poly, new_n_1kg))/max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)
				,1-(predict(N_Matrix_N_Mods_run_East$N_Matrix_N_poly,new_n_1.5kg))/max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)
				,1-(predict(N_Matrix_N_Mods_run_East$N_Matrix_N_poly,new_n_2kg))/max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)
				,1-(predict(N_Matrix_N_Mods_run_East$N_Matrix_N_poly,new_n_2.5kg))/max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)
				,1-(predict(N_Matrix_N_Mods_run_East$N_Matrix_N_poly,new_n_3kg))/max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)
				,1-(predict(N_Matrix_N_Mods_run_East$N_Matrix_N_poly,new_n_5kg))/max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)
				)
				
				c(
				max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit) ## 0% change
				,max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)-(max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)*0.05) ## 5% change
				,max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)-(max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)*0.10) ## 10% change
				,max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)-(max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)*0.20) ## 20% change
				,max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)-(max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)*0.50) ## 50% change
				,max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)-(max(N_Matrix_N_Mod_pred_East$N_Matrix_N_poly$fit)*0.80) ## 80% change
				)
