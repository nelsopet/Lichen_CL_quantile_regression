...
#Abn_For_sum34_max4_N_Lichen_Abundance_vs_N_Stats_reduc<-cbind(XVAR_names,Abn_For_sum34_max4_N_Mods_fit,Abn_For_sum34_max4_N_Mods_AIC,paste(Abn_For_sum34_max4_N_Formula))
#Abn_For_sum34_max4_N_Lichen_Abundance_vs_N_Stats_reduc<-subset(Abn_For_sum34_max4_N_Lichen_Abundance_vs_N_Stats_reduc, XVAR_names %in% Reduc_Mods_forTable)
#write.csv(Abn_For_sum34_max4_N_Lichen_Abundance_vs_N_Stats_reduc,"Abundance_Abn_For_sum34_max4_N_all_models_reduc.csv")
...
#dev.off()
  ...
  #tiff(paste("Abn_For_sum34_max4_N_Forage Lichen_vs_N_percentiles","_",format(Sys.time(),"%y%d%m"),"_nolegend.tiff",sep=""), width=1200,  height=1200, units="px", pointsize = 24)
    ...
    SOURCE_N_fPlot<-cbind(SOURCE, fit, low, high, clim_pred)
	  ...
	  plot(Abn_For_sum34_max4~N, data=SOURCE, xlab="CMAQ N kg/ha/yr", ylab="", main="90th Quantile Regression of \n Forage Lichen Abundance vs N deposition", col=Area, cex.main = 1.6, cex.lab=1.7, cex.axis = 1.7)
    ...
    points(0.08,30.08,col='red', pch=20,cex=2)
        ...
        #text(max(SOURCE$N)*0.75,max(SOURCE$Abn_For_sum34_max4)*0.65,labels=paste("R1=",print(round(Abn_For_sum34_max4_N_Mods_fit[2],2))))
        #text(max(SOURCE$N)*0.75,max(SOURCE$Abn_For_sum34_max4)*0.75,labels=paste(as.character(Abn_For_sum34_max4_N_Formula[2])), cex=1)
        #text(max(SOURCE$N)*0.75,max(SOURCE$Abn_For_sum34_max4)*0.7,labels=paste("AIC=",print(round(as.numeric(Abn_For_sum34_max4_N_Mods_AIC[2],2)))))
		#text(max(SOURCE$N)*0.7,max(SOURCE$Abn_For_sum34_max4)*0.5,labels=bquote(atop(.("Forage Lichen Abundance = 17.68 -"),.("493.88*N + 164.81*")*N^2)), cex=1.5)
		text(max(SOURCE$N)*0.5,max(SOURCE$Abn_For_sum34_max4)*0.9,labels=bquote(atop(.("Forage Lichen Abundance = 17.68 -"),.("493.88*N + 164.81*")*N^2)), cex=1.5)
...
#GetYforX<-function (x,y,ModRun,ModPred)
	...
	#}
	#GetYforX(5,Abn_For_sum34_max4_N_Mods_run,Abn_For_sum34_max4_N_poly,Abn_For_sum34_max4_N_Mod_pred)
		...
		## Heuristically solved for X given a Y using the polynomial 
			## Heuristically solved for X given a Y ...	34.58681
				## 0 % max(Abn_For_sum34_max4_N_Mod_pred$Abn_For_sum34_max4_N_poly$fit) 
				##					  Heuristically solved for X given a Y 0.08...30.08 Abn_For_sum34_max4 N kg/ha/yr
				## 5 % loss of N. Cya Heuristically solved for X given a Y 0.52...28.57 Abn_For_sum34_max4 N kg/ha/yr
				## 10% loss of N. Cya Heuristically solved for X given a Y 1.00...27.07 Abn_For_sum34_max4 N kg/ha/yr
				## 20% loss of N. Cya Heuristically solved for X given a Y 1.95...24.06 Abn_For_sum34_max4 N kg/ha/yr
				## 50% loss of N. Cya Heuristically solved for X given a Y 5.30...15.04 Abn_For_sum34_max4 N kg/ha/yr
				## 80% loss of N. Cya Heuristically solved for X given a Y 20.80...6.02  Abn_For_sum34_max4 N kg/ha/yr
				...
				c(
				 1-(predict(Abn_For_sum34_max4_N_Mods_run$Abn_For_sum34_max4_N_poly, new_n_1kg))/max(Abn_For_sum34_max4_N_Mod_pred$Abn_For_sum34_max4_N_poly$fit)
				,1-(predict(Abn_For_sum34_max4_N_Mods_run$Abn_For_sum34_max4_N_poly,new_n_1.5kg))/max(Abn_For_sum34_max4_N_Mod_pred$Abn_For_sum34_max4_N_poly$fit)
				,1-(predict(Abn_For_sum34_max4_N_Mods_run$Abn_For_sum34_max4_N_poly,new_n_2kg))/max(Abn_For_sum34_max4_N_Mod_pred$Abn_For_sum34_max4_N_poly$fit)
				,1-(predict(Abn_For_sum34_max4_N_Mods_run$Abn_For_sum34_max4_N_poly,new_n_2.5kg))/max(Abn_For_sum34_max4_N_Mod_pred$Abn_For_sum34_max4_N_poly$fit)
				,1-(predict(Abn_For_sum34_max4_N_Mods_run$Abn_For_sum34_max4_N_poly,new_n_3kg))/max(Abn_For_sum34_max4_N_Mod_pred$Abn_For_sum34_max4_N_poly$fit)
				,1-(predict(Abn_For_sum34_max4_N_Mods_run$Abn_For_sum34_max4_N_poly,new_n_5kg))/max(Abn_For_sum34_max4_N_Mod_pred$Abn_For_sum34_max4_N_poly$fit)
				)
...
#Abn_For_sum34_max4_LicheS_Abundance_vs_S_Stats_reduc<-cbind(XVAR_names,Abn_For_S_sum34_max4_Mods_fit,Abn_For_S_sum34_max4_Mods_AIC,paste(Abn_For_S_sum34_max4_Formula))
#Abn_For_sum34_max4_LicheS_Abundance_vs_S_Stats_reduc<-subset(Abn_For_sum34_max4_LicheS_Abundance_vs_S_Stats_reduc, XVAR_names %in% Reduc_Mods_forTable_S)
#write.csv(Abn_For_sum34_max4_LicheS_Abundance_vs_S_Stats_reduc,"Abundance_Abn_For_sum34_max4_S_all_models_reduc.csv")
...
#tiff(paste("Abn_For_sum34_max4_S_Forage Lichen_vs_S_percentiles","_",format(Sys.time(),"%y%d%m"),"_nolegend.tiff",sep=""), width=1200,  height=1200, units="px", pointsize = 24)
	...
	palette(c("darkorange","darkgreen"))
	plot(Abn_For_sum34_max4~S, data=SOURCE, xlab="CMAQ S kg/ha/yr", ylab="", main="90th Quantile Regression of \n Forage Lichen Abundance vs S deposition", col=Area, cex.main = 1.6, cex.lab=1.7, cex.axis = 1.7)
	#title(ylab ="Forage Lichen Abundance", cex.lab=1.7, line=2.5)
	...
	points(SOURCE_S_fPlot$S, SOURCE_S_fPlot$high, col='black', pch= 4, cex=0.25)	
  	...
  	#text(max(SOURCE$S)*0.75,max(SOURCE$Abn_For_sum34_max4)*0.65,labels=paste("R1=",print(round(Abn_For_S_sum34_max4_Mods_fit[2],2))))
  	#text(max(SOURCE$S)*0.75,max(SOURCE$Abn_For_sum34_max4)*0.7,labels=paste("AIC=",print(round(as.numeric(Abn_For_S_sum34_max4_Mods_AIC[2],2)))))
  	#text(max(SOURCE$S)*0.75,max(SOURCE$Abn_For_sum34_max4)*0.75,labels=paste(as.character(Abn_For_S_sum34_max4_Formula[2])), cex=1)
	
	#text(max(SOURCE$S)*0.5,max(SOURCE$Abn_For_sum34_max4)*0.6,labels=bquote(atop(.("Forage Lichen Abundance = 16.75 -"),.("530.25*S + 123.37*")*S^2)), cex=1.5)
	text(max(SOURCE$S)*0.5,max(SOURCE$Abn_For_sum34_max4)*0.9,labels=bquote(atop(.("Forage Lichen Abundance = 16.75 -"),.("530.25*S + 123.37*")*S^2)), cex=1.5)	  
	#legend('topright', legend=c("Raw Data East","Raw Data West", "Fitted Values Deposition only", "95% Bootstrapped Confidence Interval Deposition only","Fitted Values Climate + Deposition","0/10/20/50/80% loss"), col=c("darkorange","darkgreen","blue", "black","grey","red"), pch=16, cex=1.47)
	#legend('topright', legend=c("Raw Data East","Raw Data West", "Fitted Values Deposition only", "95% Bootstrapped Confidence Interval Deposition only","Fitted Values Climate + Deposition","0/10/20/50/80% loss"), col=c("darkorange","darkgreen","blue", "black","grey","red"), pch=16, cex=1)
	dev.off()
...
###### and the fitted polynomial line with the predicted fitted 
			...
			## x - 5
			## y - Abn_For_S_sum34_max4_Mods_run
			## ModRun - Abn_For_sum34_max4_S_poly
			## ModPred Abn_For_S_sum34_max4_Mod_pred
	...
	#}
	#
	#GetYforX(3,Abn_For_S_sum34_max4_Mods_run,Abn_For_sum34_max4_S_poly,Abn_For_S_sum34_max4_Mod_pred)
		...
		## at higher X values. Need to change denomnitor to reduced SOURCE used for plotting.
			...
			1-(predict(Abn_For_S_sum34_max4_Mods_run$Abn_For_sum34_max4_S_poly, test_s))/max(SOURCE_S_fPlot$fit)
				...
				c(
				 1-(predict(Abn_For_S_sum34_max4_Mods_run$Abn_For_sum34_max4_S_poly, new_S_1kg))/max(SOURCE_S_fPlot$fit)
				,1-(predict(Abn_For_S_sum34_max4_Mods_run$Abn_For_sum34_max4_S_poly,new_S_1.5kg))/max(SOURCE_S_fPlot$fit)
				,1-(predict(Abn_For_S_sum34_max4_Mods_run$Abn_For_sum34_max4_S_poly,new_S_2kg))/max(SOURCE_S_fPlot$fit)
				,1-(predict(Abn_For_S_sum34_max4_Mods_run$Abn_For_sum34_max4_S_poly,new_S_2.5kg))/max(SOURCE_S_fPlot$fit)
				,1-(predict(Abn_For_S_sum34_max4_Mods_run$Abn_For_sum34_max4_S_poly,new_S_3kg))/max(SOURCE_S_fPlot$fit)
				,1-(predict(Abn_For_S_sum34_max4_Mods_run$Abn_For_sum34_max4_S_poly,new_S_5kg))/max(SOURCE_S_fPlot$fit)
				)
