...
#dev.off()
    ...
    points(SOURCE_N_fPlot$N, SOURCE_N_fPlot$high, col='black', pch= 4, cex=0.25)	
		points(0.08,	18.284188, col='red', pch=20,cex=2)
	 #points(0.81,	17.369978, col='red', pch=20,cex=2)
		points(1.56,	16.455769, col='red', pch=20,cex=2)
		points(3.11,	14.627350, col='red', pch=20,cex=2)
		points(8.29,	9.142094, col='red', pch=20,cex=2)
		points(14.85,	3.656838, col='red',  pch=20,cex=2)
	...
	#text(max(SOURCE$N)*0.75,max(SOURCE$N.oligotroph.)*0.45,labels=bquote(atop(.("Count of Oligotrophs = 1.24 -"),.("286.83*N + 21*")*N^2)), cex=1.5)
	text(max(SOURCE$N)*0.65,max(SOURCE$N.oligotroph.)*0.9,labels=bquote(atop(.("Count of Oligotrophs = 1.24 -"),.("286.83*N + 21*")*N^2)), cex=1.5)
	#legend('topright', legend=c("Raw Data East","Raw Data West", "Fitted Values Deposition only", "95% Bootstrapped Confidence\n Interval Deposition only","Fitted Values Climate + Deposition","0/10/20/50/80% loss"), col=c("darkorange","darkgreen","blue", "black","grey","red"), pch=16, cex=1)
...
###### and the fitted polynomial line with the predicted fitted 
	...
	five_pct_N.oligotroph.  <- max(N.oligotroph._N_Mod_pred$N.oligotroph._N_poly$fit)-(max(N.oligotroph._N_Mod_pred$N.oligotroph._N_poly$fit)*0.05) %>% as.data.frame()
	ten_pct_N.oligotroph.   <- max(N.oligotroph._N_Mod_pred$N.oligotroph._N_poly$fit)-(max(N.oligotroph._N_Mod_pred$N.oligotroph._N_poly$fit)*0.1) %>% as.data.frame()
	twenty_pct_N.oligotroph.<- max(N.oligotroph._N_Mod_pred$N.oligotroph._N_poly$fit)-(max(N.oligotroph._N_Mod_pred$N.oligotroph._N_poly$fit)*0.2) %>% as.data.frame()
	fifty_pct_N.oligotroph. <- max(N.oligotroph._N_Mod_pred$N.oligotroph._N_poly$fit)-(max(N.oligotroph._N_Mod_pred$N.oligotroph._N_poly$fit)*0.5) %>% as.data.frame()
	eighty_pct_N.oligotroph.<- max(N.oligotroph._N_Mod_pred$N.oligotroph._N_poly$fit)-(max(N.oligotroph._N_Mod_pred$N.oligotroph._N_poly$fit)*0.8) %>% as.data.frame()
	...
	,eighty_pct_N.oligotroph.))
		...
		## x - 5
		## y - N.oligotroph._N_Mods_run
		## ModRun - N.oligotroph._N_poly
		## ModPred N.oligotroph._N_Mod_pred
	...
	#}
	#GetYforX(5,N.oligotroph._N_Mods_run,N.oligotroph._N_poly,N.oligotroph._N_Mod_pred)
		...
		## Heuristically solved for X given a Y using the polynomial 
			## Heuristically solved for X given a Y ...	34.58681
				...
				c(
				 1-(predict(N.oligotroph._N_Mods_run$N.oligotroph._N_poly, new_n_1kg))/max(N.oligotroph._N_Mod_pred$N.oligotroph._N_poly$fit)
				,1-(predict(N.oligotroph._N_Mods_run$N.oligotroph._N_poly,new_n_1.5kg))/max(N.oligotroph._N_Mod_pred$N.oligotroph._N_poly$fit)
				,1-(predict(N.oligotroph._N_Mods_run$N.oligotroph._N_poly,new_n_2kg))/max(N.oligotroph._N_Mod_pred$N.oligotroph._N_poly$fit)
				,1-(predict(N.oligotroph._N_Mods_run$N.oligotroph._N_poly,new_n_2.5kg))/max(N.oligotroph._N_Mod_pred$N.oligotroph._N_poly$fit)
				,1-(predict(N.oligotroph._N_Mods_run$N.oligotroph._N_poly,new_n_3kg))/max(N.oligotroph._N_Mod_pred$N.oligotroph._N_poly$fit)
				,1-(predict(N.oligotroph._N_Mods_run$N.oligotroph._N_poly,new_n_5kg))/max(N.oligotroph._N_Mod_pred$N.oligotroph._N_poly$fit)
				)
