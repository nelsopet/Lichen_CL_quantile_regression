...
N.oligotroph._N_Mods_run_West<-lapply(N.oligotroph._N_Formula,function (x) rq(x, tau=0.9, data=SOURCE))

	## Make null models
N.oligotroph._N_intercept_West<-rq(formula = N.oligotroph. ~ 1, tau = 0.9, data = SOURCE, model = T)

	## Grab each model fit but the intercept is hard coded 

N.oligotroph._N_Mods_fit_West<-mapply(function (y) 1-N.oligotroph._N_Mods_run_West[[y]]$rho/N.oligotroph._N_intercept_West$rho, 1:length(XVAR))

	## makes a list of mod names

N.oligotroph._N_Mod_names<-lapply(XVAR_names, function (x) paste("N.oligotroph._",paste(x),sep=""))

names(N.oligotroph._N_Mods_run_West)<-N.oligotroph._N_Mod_names

	## Save model stats as a spreadsheet of manuscript

N.oligotroph._N_Mods_AIC<-lapply(N.oligotroph._N_Mods_run_West,AIC)

N.oligotroph._N_Lichen_Count_vs_N_Stats<-cbind(XVAR_names,N.oligotroph._N_Mods_AIC,N.oligotroph._N_Mods_fit_West,paste(N.oligotroph._N_Formula))
...
N.oligotroph._N_Mod_pred_West<-mapply(MyFunc2,N.oligotroph._N_Mods_run_West)%>%as.data.frame()
...
#    text(max(SOURCE$N)*0.4,max(SOURCE$N.oligotroph.)*0.8,labels=paste("R1=",print(round(N.oligotroph._N_Mods_fit_West[i],2))))
...
#
#dev.off()
    #tiff(paste("N.oligotroph._N_Lichen_Count_vs_N_percentiles_W_US_","_",format(Sys.time(),"%y%d%m"),".tiff",sep=""), width=1200,  height=1200, units="px", pointsize = 24)
	  #jpeg(paste("N.oligotroph._N_Lichen_Count_vs_N_percentiles_W_US_",format(Sys.time(),"%y%d%m"),".jpeg",sep=""))
	  jpeg(paste("N.oligotroph._N_Lichen_Count_vs_N_percentiles_W_US_",format(Sys.time(),"%y%d%m"),"_nolegend.jpeg",sep=""))
    #tiff(paste("N.oligotroph._N_Lichen_Count_vs_N_percentiles_W_US_","_",format(Sys.time(),"%y%d%m"),"_nolegend.tiff",sep=""), width=1200,  height=1200, units="px", pointsize = 24)
    ...
    #text(max(SOURCE$N)*0.8,max(SOURCE$N.oligotroph.)*0.7,labels=paste("R1=",print(round(N.oligotroph._N_Mods_fit_West[2],2))))
	...
	#text(max(SOURCE$N)*0.65,max(SOURCE$N.oligotroph.)*0.5,labels=bquote(atop(.("Count of Oligotrophs = 13.81 -"),.("176.53*N + 60.61*")*N^2)), cex=1.5)
	text(max(SOURCE$N)*0.65,max(SOURCE$N.oligotroph.)*0.9,labels=bquote(atop(.("Count of Oligotrophs = 13.81 -"),.("176.53*N + 60.61*")*N^2)), cex=1.5)
...
###### and the fitted polynomial line with the predicted fitted 
	...
	,eighty_pct_N.oligotroph.))
		...
		## x - 5
		## y - N.oligotroph._N_Mods_run_West
	...
	#}
	#GetYforX(5,N.oligotroph._N_Mods_run_West,N.oligotroph._N_poly,N.oligotroph._N_Mod_pred_West)
			
			test_n<-as.data.frame(5)
			colnames(test_n)<-"N"
			1-(predict(N.oligotroph._N_Mods_run_West$N.oligotroph._N_poly, test_n)/max(N.oligotroph._N_Mod_pred_West$N.oligotroph._N_poly$fit))
		...
		## Heuristically solved for X given a Y using the polynomial 
			## Heuristically solved for X given a Y ...	34.58681
				...
				c(
				 1-(predict(N.oligotroph._N_Mods_run_West$N.oligotroph._N_poly, new_n_1kg))/max(N.oligotroph._N_Mod_pred_West$N.oligotroph._N_poly$fit)
				,1-(predict(N.oligotroph._N_Mods_run_West$N.oligotroph._N_poly,new_n_1.5kg))/max(N.oligotroph._N_Mod_pred_West$N.oligotroph._N_poly$fit)
				,1-(predict(N.oligotroph._N_Mods_run_West$N.oligotroph._N_poly,new_n_2kg))/max(N.oligotroph._N_Mod_pred_West$N.oligotroph._N_poly$fit)
				,1-(predict(N.oligotroph._N_Mods_run_West$N.oligotroph._N_poly,new_n_2.5kg))/max(N.oligotroph._N_Mod_pred_West$N.oligotroph._N_poly$fit)
				,1-(predict(N.oligotroph._N_Mods_run_West$N.oligotroph._N_poly,new_n_3kg))/max(N.oligotroph._N_Mod_pred_West$N.oligotroph._N_poly$fit)
				,1-(predict(N.oligotroph._N_Mods_run_West$N.oligotroph._N_poly,new_n_5kg))/max(N.oligotroph._N_Mod_pred_West$N.oligotroph._N_poly$fit)
				)
