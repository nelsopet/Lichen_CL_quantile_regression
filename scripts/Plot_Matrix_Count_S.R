...
N_Matrix_S_Mods_run<-lapply(N_Matrix_S_Formula,function (x) rq(x, tau=0.9, data=SOURCE))

	## Make null models
N_Matrix_S_intercept<-rq(formula = N_Matrix ~ 1, tau = 0.9, data = SOURCE, model = T)

	## Grab each model fit but the intercept is hard coded 

N_Matrix_S_Mods_fit<-mapply(function (y) 1-N_Matrix_S_Mods_run[[y]]$rho/N_Matrix_S_intercept$rho, 1:length(XVAR))

	## makes a list of mod names

N_Matrix_S_Mod_names<-lapply(XVAR_names, function (x) paste("N_Matrix_",paste(x),sep=""))

names(N_Matrix_S_Mods_run)<-N_Matrix_S_Mod_names

	## Save model stats as a spreadsheet of manuscript

N_Matrix_S_Mods_AIC<-lapply(N_Matrix_S_Mods_run,AIC)

N_Matrix_S_Lichen_Count_vs_S_Stats<-cbind(XVAR_names,N_Matrix_S_Mods_AIC,N_Matrix_S_Mods_fit,paste(N_Matrix_S_Formula))
...
N_Matrix_S_Mod_pred<-mapply(MyFunc2,N_Matrix_S_Mods_run)%>%as.data.frame()

	## This works but is hard coded for each predictor file. Have to change working directory each time

pdf(paste("N_Matrix_S_Lichen_Count_vs_S_percentiles","_",format(Sys.time(),"%y%d%m"),".pdf",sep=""))

MyPlot<-function (x)
    
{
    x<-N_Matrix_S_Mod_pred[i]
    ...
    text(max(SOURCE$S)*0.7,max(SOURCE$N_Matrix)*0.8,labels=paste("R1=",print(round(N_Matrix_S_Mods_fit[i],2))))
...
dev.off()
  ...
  #
  #  x<-N_Matrix_S_Mod_pred[2]
  #  x2<-unlist(x, recursive = F, use.names =T)
	#  y<-N_Matrix_S_Mod_pred[4]
...
#
  ...
  #  #text(max(SOURCE$S)*0.8,max(SOURCE$N_Matrix)*0.7,labels=paste("R1=",print(round(N_Matrix_S_Mods_fit[2],2))))
...
###### and the fitted polynomial line with the predicted fitted 
	...
	five_pct_N_Matrix  <- max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)-(max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)*0.05) %>% as.data.frame()
	ten_pct_N_Matrix   <- max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)-(max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)*0.1) %>% as.data.frame()
	twenty_pct_N_Matrix<- max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)-(max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)*0.2) %>% as.data.frame()
	fifty_pct_N_Matrix <- max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)-(max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)*0.5) %>% as.data.frame()
	eighty_pct_N_Matrix<- max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)-(max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)*0.8) %>% as.data.frame()
	...
	,eighty_pct_N_Matrix))
		...
		## x - 5
		## y - N_Matrix_S_Mods_run
		## ModRun - N_Matrix_S_poly
		## ModPred N_Matrix_S_Mod_pred
	...
	#}
	#GetYforX(5,N_Matrix_S_Mods_run,N_Matrix_S_poly,N_Matrix_S_Mod_pred)
			
			test_s<-as.data.frame(5)
			colnames(test_s)<-"S"
			1-(predict(N_Matrix_S_Mods_run$N_Matrix_S_poly, test_s)/max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit))
		...
		## Heuristically solved for X given a Y using the polynomial 
			## Heuristically solved for X given a Y ...	34.58681
				...
				colnames(new_S_5kg)<-  "S"		
							...
							c(
				 1-(predict(N_Matrix_S_Mods_run$N_Matrix_S_poly, new_S_1kg))/max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)
				,1-(predict(N_Matrix_S_Mods_run$N_Matrix_S_poly,new_S_1.5kg))/max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)
				,1-(predict(N_Matrix_S_Mods_run$N_Matrix_S_poly,new_S_2kg))/max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)
				,1-(predict(N_Matrix_S_Mods_run$N_Matrix_S_poly,new_S_2.5kg))/max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)
				,1-(predict(N_Matrix_S_Mods_run$N_Matrix_S_poly,new_S_3kg))/max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)
				,1-(predict(N_Matrix_S_Mods_run$N_Matrix_S_poly,new_S_5kg))/max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)
				)
				
				c(
				max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit) ## 0% change
				,max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)-(max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)*0.05) ## 5% change
				,max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)-(max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)*0.10) ## 10% change
				,max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)-(max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)*0.20) ## 20% change
				,max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)-(max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)*0.50) ## 50% change
				,max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)-(max(N_Matrix_S_Mod_pred$N_Matrix_S_poly$fit)*0.80) ## 80% change
				)
